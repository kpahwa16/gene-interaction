...
wandb.init(project='gene-interaction-new-cell_type-11-llama_adapted', entity='khushbu16pahwa')
import torch
...
# Setting the path
path_data = '/data1/khushbu/gene-interact/data/MTG-RNAseq/'
cell_type = "11"
# Loading the data
data_cell_type = sc.read_h5ad(os.path.join(path_data, "out", "gene_expression", 'cell_type_11.h5ad'))
# # Loading the data
...
class SwiGLU(nn.Module):
    def forward(self, x):
        gates, units = x.chunk(2, dim=-1)
        return F.silu(gates) * units
...
class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, dropout = 0.):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(dim, hidden_dim * 2, bias=False),  # Adjusted for SwiGLU
            SwiGLU(),
            # nn.GELU(),
            nn.Dropout(dropout),
...
#         print("inside forward:mask.shape", mask.shape)
       # mask_expanded = mask.unsqueeze(1).expand(-1, -1, -1, -1)
        mask_expanded = mask.unsqueeze(1).unsqueeze(1).expand(-1, self.heads, -1, -1) 
        #mask_expanded = mask.unsqueeze(1).expand(-1, self.heads, -1, -1)
        dots = torch.matmul(q, k.transpose(-1, -2)) * self.scale
#         print(dots.shape)
        # Apply masking
#         print("inside forward:dots.shape before masking", dots.shape)
        if mask is not None:
            large_negative = -1e5 # to avoid overflow error when using amp
           # dots = dots.masked_fill(~mask.unsqueeze(1).unsqueeze(2),large_negative)
            dots = dots.masked_fill(~mask_expanded, large_negative)
        ...
        # Mask the output
         ...
         #   print("out shape:", out.shape)
          #  print("mask shape:", mask.shape)
           # mask_output = mask.unsqueeze(2).expand_as(out)
            #out = out.masked_fill(~mask_output, 0)
...
def train(model, dataloader, optimizer, criterion):
    ...
    for batch_sequences, batch_labels, mask, scaling_factors_batch in dataloader:
        batch_sequences, batch_labels, mask, scaling_factors_batch = \
            [d.to(device) for d in [batch_sequences, batch_labels, mask, scaling_factors_batch]]
...
for epoch in range(num_epochs):
    ...
    if (val_accuracy > best_val_accuracy):
        ...
        best_model_path = f"/data1/khushbu/gene-interact/workdir/gene-interaction/best_models/best_model_cell_type_{cell_type}.pth"
