...
cell_type = "9"
# Loading the data
data_cell_type = sc.read_h5ad(os.path.join(path_data, "out", "gene_expression", 'cell_type_9.h5ad'))
# # Loading the data
...
embedding_dim = 128 #just for checking

# 
device_ids = [0,1,2,3,4,5,6,7]
device = torch.device("cuda:" + str(device_ids[0]) if torch.cuda.is_available() else "cpu")
model = MultiLayerModel(num_genes, embedding_dim, heads=8, dim_head= 64, mlp_dim= 64, dropout=0.)
...
for epoch in range(num_epochs):
    ...
    if not os.path.exists("/data/kpahwa/gene-interact/code/best_models/"):
        os.makedirs("/data/kpahwa/gene-interact/code/best_models/")

    if (test_accuracy > best_accuracy):
        best_accuracy = test_accuracy
        best_model_path = f"/data/kpahwa/gene-interact/code/best_models/best_model_cell_type_{cell_type}.pth"
        torch.save(model.state_dict(), best_model_path)
        print(f"  Best Model saved to {best_model_path} with test accuracy {best_accuracy:.2f}% and f1 score: {test_f1:.2f}%")
        no_improvement_count = 0
    else:
        no_improvement_count += 1

    if early_stopping and no_improvement_count >= patience:
        print(f"Early stopping after {epoch + 1} epochs due to no improvement in test accuracy.")
        break
