...
def get_second_order_grad(model, x, class_idx, num_genes):
    with torch.set_grad_enabled(True):
        ...
        grads = autograd.grad(y, x1, create_graph=True, allow_unused=True)[0].sum(dim=0)

        # This loop is a potential bottleneck
        grad_list = [autograd.grad(grad, x1, retain_graph=True)[0].detach() for grad in grads]
    ...
    with torch.no_grad():
        ...
        # Vectorized operation to reduce loop iterations
        grad_matrix0 += weight1.T.mm(grad_matrix.sum(dim=0)).mm(weight1).abs()
